# WritersLogic Browser Extensions — Publishing Guide

This directory contains packaging and publishing instructions for the WritersLogic browser extensions. All extensions share a common codebase in `apps/wld_cli/browser-extension/` and are packaged into browser-specific zip files for store submission.
...
4. Safari → Settings → Extensions → Enable "WritersLogic"
...
7. Under Privacy:
   ...
   - **Permissions justification:**
     - `nativeMessaging` — Communicates with the local WritersLogic daemon for full cryptographic witnessing
...
```sh
./apps/wld_cli/browser-extension/install-native-host.sh --chrome --extension-id YOUR_CHROME_EXTENSION_ID
```
...
5. AMO will validate the manifest. The extension ID `wld@writerslogic.com` is declared in `browser_specific_settings.gecko.id`.
...
7. **Source code submission:** AMO may request source code for review since the extension uses no build step — upload the entire `apps/wld_cli/browser-extension/` directory as a zip, or link to the public repository.
8. Upload screenshots
9. Submit for review

### Post-Publish: Native Host Registration
The Firefox extension ID is fixed (`wld@writerslogic.com`), so the native manifest works immediately:
```sh
./apps/wld_cli/browser-extension/install-native-host.sh --firefox
```
...
```sh
./apps/wld_cli/browser-extension/install-native-host.sh --edge --extension-id YOUR_EDGE_EXTENSION_ID
```
...
> **WritersLogic** creates cryptographic evidence of your writing process. As you write in Google Docs, Overleaf, Medium, or Notion, WritersLogic silently builds proof that a human authored the content — not AI.
>
> **How it works:**
> - Content checkpoints are hashed and chained together, creating a tamper-evident record of your document's evolution
> - When the WritersLogic native host is installed, the extension captures full cryptographic evidence: VDF time proofs, behavioral fingerprinting, and keystroke timing jitter
...
>
> **For full witnessing:** Install the WritersLogic desktop app and native messaging host from https://writerslogic.com
...
./apps/wld_cli/browser-extension/install-native-host.sh --all

# Or specific browsers with known extension IDs
./apps/wld_cli/browser-extension/install-native-host.sh \
...
# Install for all browsers
.\apps\wld_cli\browser-extension\install-native-host.ps1 -All

# Or specific browsers
.\apps\wld_cli\browser-extension\install-native-host.ps1 -Chrome -ExtensionId CHROME_ID
```
...
| Firefox | Add-on ID | `wld@writerslogic.com` |
...
Or manually edit:
- `apps/wld_cli/browser-extension/manifest.json` (Chrome/Edge)
- `apps/wld_cli/browser-extension/manifest-firefox.json` (Firefox)
- `apps/wld_macos/WLDSafariExtension/Resources/manifest.json` (Safari)
