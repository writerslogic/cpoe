...
This specification defines a three-tier key hierarchy for `cpop` that provides:
...
│  master_key = HKDF-SHA256(master_seed, "cpop-identity-v1", 32)          │
...
To protect the key material against a sophisticated local adversary, CPOP implements Tier 4 process-level hardening.
...
On macOS, the engine utilizes `ptrace(PT_DENY_ATTACH)` to prevent unauthorized process inspection. If a debugger attempts to attach to the cpop daemon while keys are in memory, the daemon will immediately self-terminate, protecting the integrity of the key hierarchy.
...
func InitializeIdentity(puf *hardware.PUF) (*MasterIdentity, error) {
    ...
    challenge := sha256.Sum256([]byte("cpop-identity-challenge-v1"))
    ...
    masterSeed := hkdf.Extract(sha256.New, pufResponse, []byte("cpop-identity-v1"))
