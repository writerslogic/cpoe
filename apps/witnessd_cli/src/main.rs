...
use witnessd_engine::config::WitnessdConfig;
...
use witnessd_engine::vdf::params::{calibrate, Parameters as VdfParameters};
use witnessd_engine::war;
use witnessd_engine::{derive_hmac_key, DaemonManager, SecureEvent, SecureStore};
...
EXAMPLES:\n  \
    ...
    ///
    /// View your identity fingerprint, DID, or recover from a mnemonic phrase.
    #[command(
        alias = "IDENTITY",
        alias = "Identity",
        alias = "id",
...
SECURITY:\n  \
    ...
    Identity {
        /// Show identity fingerprint (default)
        #[arg(long)]
        fingerprint: bool,
        /// Show DID (Decentralized Identifier)
        #[arg(long)]
        did: bool,
        /// Show BIP-39 mnemonic recovery phrase (DANGER: Keep private)
        #[arg(long)]
        mnemonic: bool,
        /// Recover identity from mnemonic phrase
        #[arg(long)]
        recover: bool,
        /// Mnemonic phrase for recovery (Optional: prefer stdin for security)
        #[arg(hide = true)] // Hidden to discourage usage
...
enum TrackAction {
    ...
    Start {
        ...
        file: PathBuf,
        /// Use hardware entropy when available (witnessd_jitter)
...
fn cmd_identity(
    ...
    if recover {
        ...
        // Update signing key (using first 32 bytes of PUF entropy as seed)
        // This ensures the signing key matches the identity
        let key_path = dir.join("signing_key");
        let seed = puf.get_seed(); // Assuming SoftwarePUF exposes get_seed or similar
    ...
    if json {
        ...
        if did {
            output["did"] = serde_json::Value::String(format!("did:witnessd:{}", fp));
        }

        if mnemonic {
            // Retrieve mnemonic from secure storage/PUF
            let puf_seed_path = dir.join("puf_seed");
            if puf_seed_path.exists() {
...
fn cmd_export(
    ...
    match format_lower.as_str() {
        "war" => {
            ...
            // Create and sign WAR block using the selected signer (software or hardware)
...
fn collect_declaration(
    ...
    chain_hash: [u8; 32],
    title: String,
    signer: &dyn witnessd_engine::witnessd_protocol::crypto::PoPSigner,
...
fn cmd_track_start(
    ...
    if use_witnessd_jitter {
        ...
        println!("Keystroke tracking started (witnessd_jitter mode).");
        println!("Session ID: {}", session.id);
    ...
    // Standard jitter session (no witnessd_jitter or feature not enabled)
    let jitter_params = default_jitter_params();
    ...
    match action {
        ...
        TrackAction::Stop => {
            ...
            if is_hybrid {
                ...
                let mut session = witnessd_engine::HybridJitterSession::load(&session_path, None)
                ...
                println!("Tracking session stopped (witnessd_jitter mode).");
        ...
        TrackAction::Status => {
            ...
            if is_hybrid {
                let session_path = tracking_dir.join(format!("{}.hybrid.json", session_id));
                let session = witnessd_engine::HybridJitterSession::load(&session_path, None)
                    .map_err(|e| anyhow!("Error loading hybrid session: {}", e))?;
                ...
                println!("=== Active Tracking Session (witnessd_jitter) ===");
                println!("Session ID: {}", session.id);
        ...
        TrackAction::List => {
            ...
            for session in hybrid_sessions {
                ...
                let phys_ratio = session.phys_ratio();
                println!(
                    "  {} [witnessd_jitter]: {} keystrokes, {} samples, {:?}, {:.0}% hardware",
                    session.id,
                    session.keystroke_count(),
                    session.sample_count(),
                    duration,
...
fn cmd_status() -> Result<()> {
    ...
    // Check hardware provider
    let tpm_provider = tpm::detect_provider();
    let caps = tpm_provider.capabilities();
    if caps.hardware_backed {
        println!("Hardware HSM: Available ({})", tpm_provider.device_id());
    } else {
        println!("Hardware HSM: None (Software-only)");
    }
...
async fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Identity {
            ...
            cmd_identity(fingerprint, did, mnemonic, recover, phrase, json)?;
        }
