...
/// Atomically persist the session via tmp+rename.
fn save_session(session_file: &std::path::Path, session: &PresenceSession) -> Result<()> {
    ...
    fs::rename(&tmp_path, session_file).with_context(|| "Failed to finalize session file")?;
    Ok(())
}
        ...
        PresenceAction::Challenge => {
            ...
            let final_session = if conflict {
                ...
                let disk_data = fs::read(&session_file)
                    .with_context(|| "Failed to re-read session after concurrent modification")?;
