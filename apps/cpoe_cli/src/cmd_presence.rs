...
fn acquire_session_lock(session_file: &Path) -> Result<fs::File> {
    ...
    cpoe::restrict_permissions(&lock_path, 0o600).context("restrict lock file permissions")?;
...
fn load_session(session_file: &std::path::Path) -> Result<PresenceSession> {
    let data = fs::read(session_file)
        .map_err(|_| anyhow!("No active session. Run 'cpoe presence start' first."))?;
    ...
    match action {
        PresenceAction::Start => {
            ...
            if session_file.exists() {
                drop(lock_file);
                return Err(anyhow!(
                    "Session already active. Run 'cpoe presence stop' first."
                ));
            }
            ...
            if out.json {
                ...
                println!();
                println!("Run 'cpoe presence challenge' periodically to verify presence.");
            }
        }
