...
/// CPOP evidence packets are natural Signed Statements.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct SignedStatement {
    /// The COSE_Sign1 encoded evidence packet.
    pub envelope: Vec<u8>,
    /// Content type of the payload (application/vnd.writersproof.cpop+cbor).
...
/// Convert a CPOP evidence packet into a SCITT Signed Statement.
///
/// The evidence CBOR bytes become the envelope, the document hash is hex-encoded
/// as the subject identifier, and the content type is set to the CPOP media type.
...
/// Convert a CPOP beacon attestation to a SCITT-compatible receipt structure.
///
