...
use cpop_engine::utils::stats::{coefficient_of_variation, mean, std_dev};
...
fn bench_event_hashing(c: &mut Criterion) {
    ...
    c.bench_function("compute_event_hash", |b| {
        b.iter(|| compute_event_hash(black_box(&data)))
    });

    let hmac_key = [0x42; 32];
    c.bench_function("compute_event_hmac", |b| {
        b.iter(|| compute_event_hmac(black_box(&hmac_key), black_box(&data)))
    });
}
...
fn bench_stats(c: &mut Criterion) {
    ...
    c.bench_function("stats_cv", |b| {
        b.iter(|| coefficient_of_variation(black_box(&data)))
    });
}
