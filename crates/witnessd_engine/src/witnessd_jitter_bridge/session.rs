...
use crate::witnessd_jitter_bridge::doc_tracker::DocumentTracker;
use crate::witnessd_jitter_bridge::types::{
    EntropyQuality, HybridEvidence, HybridSample, HybridSessionData,
};
use crate::witnessd_jitter_bridge::zone_engine::ZoneTrackingEngine;
...
pub struct HybridJitterSession {
    pub(crate) witnessd_jitter_session: PhysSession,
...
impl HybridJitterSession {
    pub fn new(
        ...
        let witnessd_jitter_session = PhysSession::new(secret);

        Ok(Self {
            witnessd_jitter_session,
    ...
    pub fn record_keystroke(&mut self, keycode: u16) -> Result<(u32, bool), String> {
        ...
        let jitter = self
            .witnessd_jitter_session
            .sample(&input)
            .map_err(|e| format!("witnessd_jitter sample failed: {e}"))?;
    ...
    pub fn phys_ratio(&self) -> f64 {
        self.witnessd_jitter_session.phys_ratio()
    }

    pub fn entropy_quality(&self) -> EntropyQuality {
        let evidence = self.witnessd_jitter_session.evidence();
    ...
    pub fn export(&self) -> HybridEvidence {
        ...
        HybridEvidence {
            ...
            witnessd_jitter_evidence: self.witnessd_jitter_session.export_json().ok(),
        }
    }
    ...
    pub fn load(path: impl AsRef<Path>, key_material: Option<[u8; 32]>) -> Result<Self, String> {
        ...
        Ok(Self {
            witnessd_jitter_session: PhysSession::new(secret),
...
impl HybridEvidence {
    pub fn verify(&self) -> Result<(), String> {
        ...
        if let Some(ref witnessd_jitter_json) = self.witnessd_jitter_evidence {
            self.verify_witnessd_jitter_evidence(witnessd_jitter_json)?;
        }

        Ok(())
    }

    fn verify_witnessd_jitter_evidence(&self, json: &str) -> Result<(), String> {
        let chain: PhysEvidenceChain = serde_json::from_str(json)
            .map_err(|e| format!("witnessd_jitter evidence parse error: {e}"))?;

        if !chain.validate_sequences() {
            return Err("witnessd_jitter evidence: sequence numbers not monotonic".to_string());
        }

        if !chain.validate_timestamps() {
            return Err("witnessd_jitter evidence: timestamps not monotonic".to_string());
        }
