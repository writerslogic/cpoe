...
Vendors should generally use the **[[Glossary#Sentinel|Sentinel]]** to handle background capture:
1. `StartWitnessing(path)`: Tells the daemon to begin monitoring a specific file.
2. `StopWitnessing(path)`: Ends the capture session and flushes the [[Glossary#WAL|WAL]].
