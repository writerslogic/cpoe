...
fn test_interval_to_bucket() {
    ...
    assert_eq!(interval_to_bucket(Duration::from_secs(1)), 9); 
}
...
fn test_jitter_engine() {
    ...
    let doc_hash = [1u8; 32];
    let (jitter1, sample1) = engine.on_keystroke(0x0C, doc_hash); 

    assert!(jitter1 >= MIN_JITTER && jitter1 <= MAX_JITTER);
    assert!(sample1.is_some());

    let (jitter2, sample2) = engine.on_keystroke(0x0D, doc_hash); 
...
fn test_jitter_engine_invalid_keycode() {
    ...
    let (jitter, sample) = engine.on_keystroke(0xFF, [0u8; 32]); 
    assert_eq!(jitter, 0);
...
fn test_is_human_plausible() {
    ...
    profile2.hand_alternation = 0.05; 
