...
pub(crate) fn cmd_init() -> Result<()> {
    ...
    if !puf_seed_path.exists() {
        ...
        let puf =
            SoftwarePUF::new_with_path(&puf_seed_path).map_err(|e| anyhow!("PUF seed: {}", e))?;
        ...
        let puf =
            SoftwarePUF::new_with_path(&puf_seed_path).map_err(|e| anyhow!("load PUF: {}", e))?;
        let identity =
    ...
    if !db_path.exists() {
        ...
        let hmac_key = if let Ok(Some(key)) = SecureStorage::load_hmac_key() {
            ...
            derive_hmac_key(&priv_key.to_bytes()).to_vec()
        };
        let _db = SecureStore::open(&db_path, hmac_key).context("create database")?;
