...
use crate::codec::cbor::MAX_CBOR_PAYLOAD;
...
pub fn encode_to<T: Serialize, W: Write>(value: &T, writer: W) -> Result<()> {
    serde_json::to_writer_pretty(writer, value).map_err(|e| CodecError::JsonEncode(e.to_string()))
}

/
pub fn encode_to_compact<T: Serialize, W: Write>(value: &T, writer: W) -> Result<()> {
    serde_json::to_writer(writer, value).map_err(|e| CodecError::JsonEncode(e.to_string()))
}

/
pub fn decode_from<T: DeserializeOwned, R: Read>(reader: R) -> Result<T> {
    let limited = reader.take(MAX_CBOR_PAYLOAD as u64);
    serde_json::from_reader(limited).map_err(|e| CodecError::JsonDecode(e.to_string()))
}
