...
pub(crate) mod lock_order {
    ...
    pub fn assert_order(level: u8) {
        MAX_HELD.with(|cell| {
            ...
            debug_assert!(
                held < level,
                "Lock ordering violation (AUD-041): acquiring level {level} \
                 while level {held} is held. Order: signing_key(1) < sessions(2)."
            );
            cell.set(level);
        });
    }
...
impl Sentinel {
        ...
        *self.shutdown_tx.lock_recover() = Some(shutdown_tx);
        ...
        tokio::spawn(async move {
            ...
            let mut idle_check_interval =
                interval(Duration::from_secs(config.idle_check_interval_secs));
    ...
    pub fn start_witnessing(
        ...
        let signing_key_clone = self.signing_key.read_recover().clone();
        ...
        if hex::decode_to_slice(hex_str, &mut session_id_bytes).is_ok() {
            if let Some(key) = signing_key_clone {
