...
//! a text field, not on disk. Sessions use `ephemeral:
...
pub fn ffi_ephemeral_checkpoint(session_id: String, content: String, message: String) -> FfiResult {
    ...
    let ephemeral_path = format!("ephemeral:
...
pub fn ffi_ephemeral_checkpoint_hash(
    ...
    if let Some(last) = entry.last_checkpoint_at {
        if Instant::now().duration_since(last) < MIN_CHECKPOINT_INTERVAL {
            return FfiResult {
                success: false,
                message: None,
                error_message: Some("Checkpoint rate limited (min 1s interval)".to_string()),
...
mod tests {
    ...
    fn test_ephemeral_inject_jitter() {
        ...
        let intervals = vec![50_000, 80_000, 120_000, 5, 15_000_000]; 
