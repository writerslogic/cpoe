...
fn t06_export_cpop() {
    ...
    assert!(data.len() > 50, "CPOP too small");
}
...
fn t08_log_ordering() {
    ...
    let log = cpop_engine::ffi::system::ffi_get_log(doc);
...
fn t10_hmac_recovery() {
    ...
    assert!(cpop_engine::ffi::system::ffi_get_status().initialized);
}
...
fn t13_calibration() {
    let (_dir, _g) = setup();
    assert!(cpop_engine::ffi::system::ffi_init().success);
    let cal = cpop_engine::ffi::forensics::ffi_calibrate_swf();
