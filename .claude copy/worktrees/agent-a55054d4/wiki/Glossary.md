...
Key terms used throughout the CPOP documentation.
...
### Attestation
A cryptographic statement by a device or software component that a particular set of conditions holds. In CPOP, attestation binds process evidence to hardware trust anchors (e.g., TPM, Secure Enclave).

### Adversarial Collapse
The core security property of CPOP: any attempt to forge process evidence requires the adversary to expend effort comparable to actually performing the creative process. The cost of forgery *collapses* to near the cost of genuine authorship.
...
### Checkpoint
A snapshot of a document's state at a specific point in time, cryptographically bound to VDF proofs and the Merkle Mountain Range. Created via `cpop commit`.

### COSE (CBOR Object Signing and Encryption)
The encoding format used for CPOP evidence packets, as specified in RFC 9052. Provides compact, binary-safe serialization with built-in signature support.
...
### Falsifiability
The property that CPOP evidence can be independently tested and potentially disproven. Unlike unfalsifiable claims, CPOP evidence provides concrete, testable assertions.

## H

### HMAC (Hash-based Message Authentication Code)
A keyed hash function used in CPOP for tamper detection on evidence chains and for the "economic security" model in jitter computation.
...
An append-only data structure used by CPOP to store event hashes. Provides efficient proof of inclusion and tamper detection. Unlike a standard Merkle tree, MMR supports efficient appending.

## P

### PoP (Proof-of-Process)
The protocol implemented by CPOP, currently being socialized in the IETF RATS working group. PoP provides verifiable evidence that a particular process (e.g., human typing) occurred as claimed.

### PUF (Physically Unclonable Function)
Hardware-derived entropy sources that produce outputs unique to a specific physical device. Used by cpop_jitter in "physics" mode to bind evidence to hardware.
...
### Roughtime
A protocol for rough time synchronization that provides cryptographic proof of time. Used by CPOP as an external time anchor.

## S

### Sentinel
The CPOP component responsible for real-time file monitoring, change detection, and session management. Watches configured directories and triggers checkpoints.
...
A hardware security module that provides cryptographic operations in a tamper-resistant environment. CPOP uses TPM 2.0 for hardware-backed key storage and attestation on Linux.

### TSC (Time Stamp Counter)
A hardware register in modern CPUs that increments with each clock cycle. Used by cpop_jitter for hardware entropy collection in "physics" mode.

## V

### VDF (Verifiable Delay Function)
A function that takes a specified amount of time to compute but whose result can be quickly verified. CPOP uses VDFs to create cryptographic proofs that a minimum amount of wall-clock time has elapsed between checkpoints.
...
CPOP's fault-tolerance mechanism built on WAL. WAR blocks contain enough information to recover session state after crashes or unexpected termination.
